...
*Setup instructions for a gitian build of 1UP using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 1UP
...
Clone the git repositories for 1up and gitian and then checkout the 1up version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/1up-crypto/1up.git
cd 1up
...
Follow the instructions in [doc/release-process.md](release-process.md) in the 1up repository
...
To build 1UP (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the 1up repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/1up/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/1up-crypto/1up
...
For example:
```bash
URL=https://github.com/crowning-/1up.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit 1up=${COMMIT} --url 1up=${URL} ../1up/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 1up=${COMMIT} --url 1up=${URL} ../1up/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 1up=${COMMIT} --url 1up=${URL} ../1up/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/1up-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/1up-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/1up-build.assert
```
...
[1up/gitian.sigs](https://github.com/1up-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
