...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymize1UPAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymize1UPAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymize1UPAmount = strAnonymize1UPAmount.remove(strAnonymize1UPAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymize1UPAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymize1UPAmount * COIN) nMaxToAnonymize = nAnonymize1UPAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymize1UPAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymize1UPAmount));
        strAnonymize1UPAmount = strAnonymize1UPAmount.remove(strAnonymize1UPAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymize1UPAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
