...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("1UPMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("1UPMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("1UPMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("1up-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running 1UPMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
